			...
			if(!validEnvironment)
			{
				if(RenderHelper.isPointWithinRectangle(params.renderX, params.renderY, 20, 20, cursorX - x, cursorY - y))
				{
					List<FormattedCharSequence> lines = List.of(
							MIEText.WATER_PUMP_ENVIRONMENT_0.text().withStyle(ChatFormatting.RED).getVisualOrderText(),
							MIEText.WATER_PUMP_ENVIRONMENT_1.text().withStyle(ChatFormatting.RED).getVisualOrderText()
					);
